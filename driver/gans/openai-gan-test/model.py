...
filename = "imagenet_train_128.tfrecords"
...
class DCGAN(object):
    def __init__(self, sess, image_size=108, is_crop=True,
        ...
        if predictor_func is None:
            predictor_func = default_predictor
        self.predictor = Predictor(self, predictor_func)  # 自己加的预测的函数
    ...
    def save(self, checkpoint_dir, step):
        ...
        self.saver.save(self.sess,
                        ...
                        global_step=step)  #保存模型

    def load(self, checkpoint_dir):
        print(" [*] Reading checkpoints...")

        self.saver = tf.train.Saver() #我加的
        model_dir = "%s_%s" % (self.dataset_name, self.batch_size)
        checkpoint_dir = os.path.join(checkpoint_dir, model_dir)
...
class Predictor(object):
    """    
    仿照他写的一个预测的函数，对于测试集进行数据的预测
    也是一个类，定义之后可以通过函数的形式进行调用
    """
    def __init__(self, dcgan, func):
        self.dcgan = dcgan
        self.func = func
