...
filename = 'imagenet_train_labeled_' + str(IMSIZE) + '.tfrecords'
# filename = 'imagenet_train_labeled_forTest' + str(IMSIZE) + '.tfrecords'
...
def build_model(self):
    ...
    all_g_grads = []
    config = self.config
    d_opt = tf.train.AdamOptimizer(config.discriminator_learning_rate, beta1=config.beta1) #原版
    ...
    #     g_opt = tf.train.AdamOptimizer(config.generator_learning_rate, beta1=config.beta1)
    # with tf.variable_scope(tf.get_variable_scope()): #原版没有，我加的，注意，如果要恢复原版需要将下面的移动格子
    ...
    # with tf.variable_scope(tf.get_variable_scope(), reuse=False):  #我加的
...
def build_model_single_gpu(self, gpu_idx):
    ...
    if gpu_idx == 0:
        ...
        self.test_images = tf.placeholder(tf.float32, shape=[8, 128, 128, 3],
                                        name='test_images')  #我增加的用于输出测试图片的一个张量输入
    ...
    class_logits, D_on_data, D_on_data_logits, D_on_G, D_on_G_logits ,_= self.discriminator(joint, reuse=True, prefix="joint ")
